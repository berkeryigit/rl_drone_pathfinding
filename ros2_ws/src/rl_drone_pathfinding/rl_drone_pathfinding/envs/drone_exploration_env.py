...
Odul:
    +1.0 yeni voxel
    +15  yeni oda
    -0.3 * max(0, 1 - min_lidar/2.0)  → progressive duvar cezasi (0-2m arasi)
    -0.5 duvara cok yakin (<0.5m)
    -10  carpisme (episode biter)
    -0.1 ayni voxelde kalma
...
WALL_PENALTY_DIST   = 2.0   # bu mesafeden yakinsa progresif ceza baslar
STEP_DT = 0.02
...
class DroneExplorationEnv(gym.Env):
    ...
    def __init__(self, world_name=DEFAULT_WORLD_NAME, drone_name=DEFAULT_DRONE_NAME,
                 max_episode_steps=1000, seed=None):
        super().__init__()
    ...
    def step(self, action: np.ndarray):
        ...
        # Progressive duvar cezasi: 2m'den itibaren giderek artar
