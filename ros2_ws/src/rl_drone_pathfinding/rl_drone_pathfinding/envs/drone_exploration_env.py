...
class DroneExplorationEnv(gym.Env):
    ...
    def __init__(self, world_name=DEFAULT_WORLD_NAME, drone_name=DEFAULT_DRONE_NAME,
                 max_episode_steps=1000, seed=None, env_id=0, discrete=False):
        super().__init__()
        ...
        # Aksiyon uzayi:
        #   discrete=False (PPO/TD3 — Berker): surekli [v, w] in [-1,1]^2
        #   discrete=True  (A3C/DQN):          Discrete(4) -> ileri/sol/sag/dur
        #     0=ILERI (v=V_MAX, w=0), 1=SOLA (v=0.5*V_MAX, w=+W_MAX),
        #     2=SAGA  (v=0.5*V_MAX, w=-W_MAX), 3=DUR (v=0, w=0)
        if discrete:
    ...
    def step(self, action):
        ...
        # forward_act gercek ileri hizdan turetildi -> hem surekli hem discrete'te calisir.
        forward_open = float(lidar_obs[FORWARD_BIN])
        forward_act  = max(0.0, v) / V_MAX
