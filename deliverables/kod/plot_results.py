...
algo/td3 (SAC) teslimindeki plot_results.py ile AYNI yapi; etiketler PPO'ya gore
(actor = policy gradient loss, critic = value loss, ajan = PPO).
...
Grafikler:
  ...
  3) loss_egrisi.png           -> policy gradient (actor) & value (critic) loss
  4) hiperparametre_duyarlilik.png -> >=2 param x >=3 deger (runs_hp/ varsa)
  5) baseline_karsilastirma.png -> random/heuristic vs egitilmis PPO ajani
...
def graph_learning(dfs, out_path: Path):
    ...
    ax.plot(centers, mean_s, color="#0288d1", linewidth=2.2, label="Ortalama episode getirisi (PPO)")
    ax.axhline(0, color="gray", linestyle="--", linewidth=0.8)
    _style(ax, "1) Ogrenme Egrisi (PPO egitim)", "Kumulatif adim (timestep)", "Episode getirisi (toplam odul)", n)
    _save(fig, out_path)
...
def graph_eval_curve(runs_root: Path, seeds: list[int], out_path: Path):
    ...
    ax.plot(grid, mean, color="#2e7d32", linewidth=2.2, label="Deterministik eval getirisi (PPO)")
    ax.axhline(0, color="gray", linestyle="--", linewidth=0.8)
...
# --------------------------------------------------------------------------- #
# 3) Loss egrisi (PPO: policy gradient (actor) & value (critic))
# --------------------------------------------------------------------------- #
def graph_loss(dfs, out_path: Path):
    ...
    if a is not None:
        ...
        ax.plot(ca, ma, color="#c62828", linewidth=2, label="Policy gradient loss (actor)")
    ...
    if c is not None:
        ...
        ax2.plot(cc, mc, color="#6a1b9a", linewidth=2, label="Value loss (critic)")
        ax2.set_ylabel("Value loss (critic)", color="#6a1b9a")
        ax2.tick_params(axis="y", labelcolor="#6a1b9a")
    ax.set_title("3) Loss Egrisi (PPO: policy gradient & value loss)", fontsize=12, fontweight="bold")
...
def graph_hp(hp_root: Path, out_path: Path):
    ...
    Her parametre kendi alt-grafiginde (subplot) cizilir: olcekleri (lr ~1e-4,
    ent_coef ~0..0.05) farkli oldugu icin tek eksende karistirilmaz. Boylece
    ent_coef=0.0 noktasi da net gosterilir (log eksende kaybolmaz).
    """
    if not hp_root.exists():
        ...
        return
    param_dirs = sorted(p for p in hp_root.iterdir() if p.is_dir())
    data = {}
    for param_dir in param_dirs:
        ...
        if xs:
            order = np.argsort(xs)
            data[param_dir.name] = (np.array(xs)[order], np.array(ys)[order], np.array(es)[order])
    if not data:
        _placeholder(out_path, "4) Hiperparametre Duyarliligi", f"{hp_root} icinde gecerli kosu yok")
        return

    n = len(data)
    colors = {"learning_rate": "#ef6c00", "ent_coef": "#1565c0"}
    fig, axes = plt.subplots(1, n, figsize=(6.0 * n, 5.4))
    if n == 1:
        axes = [axes]
    for ax, (name, (xs, ys, es)) in zip(axes, data.items()):
        ax.errorbar(xs, ys, yerr=es, marker="o", markersize=8, capsize=5, linewidth=2,
                    color=colors.get(name, "#555"))
        ax.set_title(name, fontsize=12, fontweight="bold")
        ax.set_xlabel(f"{name} degeri")
        ax.set_ylabel("Final episode getirisi (mean +/- std)")
        ax.grid(alpha=0.25, which="both")
        if name == "learning_rate":
            ax.set_xscale("log")
        else:
            # ent_coef: lineer eksen (0.0 dahil); noktalari ayri tut
            pad = max(1e-3, (xs.max() - xs.min()) * 0.15)
            ax.set_xlim(xs.min() - pad, xs.max() + pad)
        for x, y in zip(xs, ys):
            ax.annotate(f"{y:.0f}", (x, y), textcoords="offset points", xytext=(0, 10),
                        ha="center", fontsize=9)
    fig.suptitle("4) Hiperparametre Duyarliligi (final episode getirisi, 3 seed) "
                 "-- dusuk lr/ent => somuru, yuksek => kesif",
...
def graph_baseline(eval_csv: Path, baseline_csv: Path, out_path: Path):
    ...
    if eval_csv.exists():
        ...
        groups[AGENT_LABEL] = e["ep_return"].to_numpy(dtype=float)
    if not groups:
        _placeholder(out_path, "5) Baseline Karsilastirma",
                     "baseline/eval csv yok -> baseline.py ve evaluate.py calistirin")
        return
    order = [k for k in ("random", "heuristic", AGENT_LABEL) if k in groups]
