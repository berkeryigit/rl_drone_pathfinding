"""A3C (A2C, discrete) policy'sini değerlendir + kapsama/yörünge/çarpışma figürleri.
PPO eval_fast.py ile AYNI çıktı formatı (adil kıyas için).
    python3 fast_sim/eval_a3c.py --model runs/a3c_v1/checkpoints/a3c_drone_final.zip \
            --episodes 100 --version a3c_v1 --lidar-history 2
"""
...
from fast_drone_env import (WALLS, SPAWN_X, SPAWN_Y, WORLD_HALF,  # noqa: E402
                            GRID_NXY, N_ROOMS)
...
def main(argv=None):
    ...
    ap.add_argument("--version", default="a3c_v1")
    ap.add_argument("--max-steps", type=int, default=2500)
    ap.add_argument("--lidar-history", type=int, default=1)
    a = ap.parse_args(argv)

    env = FastDroneEnvDiscrete(max_episode_steps=a.max_steps, lidar_history=a.lidar_history)
    model = A2C.load(a.model)   # norm_obs=False -> ham obs, vecnormalize gerekmez
    print(f"[eval_a3c] {a.model} | {a.episodes} ep | DISCRETE")
    ...
    ax.set_title(f"{a.version} (A3C/discrete, numpy) KAPSAMA — {a.episodes} ep | birlesik {cov_cells}/{TOTAL}")
    ...
    ax.set_title(f"{a.version} (A3C/discrete, numpy) YORUNGELER + carpisma"); ax.legend(loc="upper right")
    ...
    md = [f"# {a.version} (A3C / discrete, numpy sim) — {a.episodes} episode", "",
          f"Model: `{Path(a.model).name}` | deterministic | hareketli engeller aktif | A2C(~A3C)", "",
          "| Metrik | Deger |", "|---|---|",
          f"| Return | {st(rets)} |",
